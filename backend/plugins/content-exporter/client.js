...
// What it does
// ────────────
//   * Adds an "Export Pack" GM tab with creature/spell pickers.
...
// the host library, and pushes any bundled treasure items into the GM
...
// because the chest is ephemeral GM state — closing the session
...
async function importTreasure(dataApi) {
  // Treasure chest is purely GM-side state; only the GM client has the
  ...
  async register({ React, registries, context }) {
    ...
    (async () => {
      ...
      try {
        ...
        try { await importTreasure(data); } catch { /* GM-only, non-fatal */ }
...
// exporter UI reflects GM additions to the chest without a refresh.
// Only available on the GM client — on the player view the host
...
function attachTreasureBridge() {
  ...
  if (typeof ns.subscribe === 'function') {
    unsubTreasure = ns.subscribe((next) => {
      ...
      // Drop selections for items the GM has removed from the chest.
  ...
  async register({ React, registries, context }) {
    ...
    function ExportTab() {
      ...
      return React.createElement('div',
        ...
        // Treasure pulled live from the host's chest. The chest is GM-
        // ephemeral (no DB) — the host exposes window.__tabletopForge.
        // treasure for cross-plugin access. Items the GM removes from
        ...
        section === 'treasure' && React.createElement('div',
          ...
          treasure.length === 0
            ? React.createElement('div', { className: 'text-xs text-gray-500 italic px-2 py-2' },
                'The treasure chest is empty. Add items via the Treasure tab in the GM panel, then come back here.')
