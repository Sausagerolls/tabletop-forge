...
// ── Native-build static frontend serve ───────────────────────────────
// In Docker, nginx serves the React build and proxies /api/ + /socket.io/
// over to this process. The native build collapses both into one Express
// instance so there's a single port and no nginx dependency. Looks at
// `../frontend/dist` (the Vite build output, gitignored) and serves
// every non-API path from there. The SPA fallback at the bottom hands
// any unknown route back to index.html so React Router owns the URL.
const FRONTEND_DIST = path.resolve(__dirname, '..', '..', 'frontend', 'dist');
if (fs.existsSync(FRONTEND_DIST)) {
  app.use(express.static(FRONTEND_DIST));
} else {
  console.warn(
    `[native] frontend/dist missing at ${FRONTEND_DIST}. ` +
    `Run \`npm run build\` in native-fork/frontend before starting the server.`
  );
}
...
// SPA fallback — every unmatched non-API GET returns the React app's
// index.html. Has to live after every router so /api/* + /socket.io/*
// claim their own paths first. Skips routes starting with /api,
// /uploads, /sounds, /socket.io to keep their 404s actionable.
if (fs.existsSync(FRONTEND_DIST)) {
  app.get(/^(?!\/(api|uploads|sounds|socket\.io)\b).*/, (req, res) => {
    res.sendFile(path.join(FRONTEND_DIST, 'index.html'));
  });
}

const PORT = process.env.PORT || 3001;
// Wait for the PGlite schema bootstrap before binding. The original
// Docker stack has a `depends_on: db: condition: service_healthy`
// that handles this; the native build replaces it with this explicit
// await so the first request never races against init.sql.
db.ready.then(() => server.listen(PORT, async () => {
